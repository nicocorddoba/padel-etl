...
def winner_calculation(team_1_scores: tuple, team_2_scores: tuple, dict_flag: dict = None) -> str:
    if dict_flag:
...
def tournament_details(page: Page, tournament_dict:dict, gender_arg:str):
    ...
    for k in range(int(days)):
        ...
        page.goto(new_link + f"{day}?t=tol")
        ...
        for j in range(game_div.count()):
            ...
            round_id_dict = {"Q1":"Q1", "Q2": "Q2",'Round of 64':64, 'Round of 32':32, 'Round of 16':16, 'Quarterfinals':8, 'Semifinals':4, 'Final':2}
            
            if round_name in rounds and gender == gender_arg:
                # Extracter re pattern for team names and scores
                pattern_names = r"([A-Z]\.\s[A-Za-z]+(?:\s\(\d+\))?)"
                ...
                match_dict = {"id":match_id,
                                ...
                                "winner": winner_calculation(team_1_scores, team_2_scores, dict_flag)}
                # print(match_dict)
